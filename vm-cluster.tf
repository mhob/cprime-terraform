...
# Add an association between the virtual machines and the load balancer. We use
# `count` to create a separate association for each virtual machine.
...
# availability set for the entire cluster.
...
# Add the virtual machine. We use `count` here to create multiple virtual
# machines. Note that the VMs are identical other than references to
# `count.index`.
